...
. Initialize stem:[Q(s,a)] for all state-action pairs (commonly to 0)
. At state stem:[s_t], select an action stem:[a_t] using an stem:[\epsilon-] greedy policy.
. Execute stem:[a_t]. Observe the reward stem:[r_{t+1}] and the next state stem:[s_{t+1}]
. Apply the Q-learning update rule
. Set stem:[s_t \leftarrow s_{t+1}]
. Repeat from Step 2 until the episode ends (terminal state or time limit)
. Continue the process over many episodes until the Q-values converge or performance stabilizes.

The policy that emerges when the Q-table converges is the optimal policy: stem:[\pi_*(s) = \arg \max_a Q(s,a)].
...
We can implement an afterstate value function which would update both the state-action pairs stem:[A] and stem:[B] after we reach state in stem:[C]. An afterstate value function is a function that updates all the state-action pairs that result in the same afterstate. Implementing this in our algorithm makes the learning and convergence faster.
