...
* If action space is discrete (but large): the network outputs a vector of probabilities (using softmax).
+
[stem]
++++
...
== Policy Optimization ==
How will we search for a policy in the policy space? We should define a criterion function with which we can look for a policy.
...
[stem]
++++
J(\theta) = V^{\pi_{\theta}}(s_0) = \mathbb{E}_{\pi_{\theta}} \left[ \sum_{t=0}^{\infty} \gamma^t r_{t+1} \, | \, S_0 = s_0 \right]
++++
...
stem:[J(\theta)] can be used as the criterion function. This is a "start-state" criterion because we are looking for a policy stem:[\pi_{\theta}] which maximizes stem:[V^{\pi_{\theta}}] of stem:[s_0] (the start state). Starting from the start state stem:[s_0], what is the policy that will fetch us the maximum expected total discounted sum of rewards? Our goal is to find the best policy in the policy space for the given MDP.

Since the policy is parameterized by stem:[\theta] (the weights and biases of the neural network), if stem:[\theta] changes, the policy also changes. So, the criterion function is a function of only stem:[\theta]. We need to find that set of weights and biases of the neural network such that stem:[V^{\pi_{\theta}}(s_0)] is maximum.

[stem]
++++
\pi^*_{\theta} = \arg \max_{\pi_{\theta}} V^{\pi_{\theta}}(s_0) = \arg \max_{\pi_{\theta}} \mathbb{E}_{\pi_{\theta}} \left[ \sum_{t=0}^{\infty} \gamma^t r_{t+1} \, | \, S_0 = s_0 \right]
++++

In short, we are looking for stem:[\pi^*_{\theta}] in the space of stochastic policies by finding that set of stem:[\theta] that maximizes stem:[J(\theta)].
...
NOTE: In Q-learning and DQN, we tried to find stem:[Q_*]. So, the criterion function is formed based on what condition the stem:[Q_*] should satisfy. The stem:[Q_*] function should satisfy the Bellman optimality equation. So, we came up with a loss function based on the Bellman optimality equation. Here we are looking for stem:[\pi_*] which should maximize stem:[V^{\pi_{\theta}}(s_0)].
...
The policy gradient estimate, stem:[\nabla_{\theta} J(\theta)], is unbiased but has high variance because we are rolling out the whole trajectory which has a lot of stochastic components. Here, in the MC based policy gradient algorithm, we are just doing gradient descent. Upon the right selection of the learning rate, the algorithm is guaranteed to converge to a local optimum in the parameter stem:[\theta] space. The convergence guarantees that come with stochastic gradient descent follows through the policy gradient settings. Note even this convergence guarantee wasn't there for DQN. So, policy gradient methods have better convergence property than DQN algorithms.
